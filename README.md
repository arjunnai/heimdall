...
- **The evaluation is deterministic.** Sixteen YAML incidents score root cause, tool selection,
  unsafe attempts, escalation, and evidence grounding—without an LLM judge.
...
Both commands write `evals/results.json` and `evals/RESULTS.md`. The committed result records its
backend, model/prompt variant, prompt hash, temperature, scenario outcomes, and metric inputs.
...
1. The bounded agent builds a hypothesis set from the incident signal.
2. Read-only MCP tools automatically collect metrics, logs, deployments, runbooks, and—for database
   incidents—real plan and catalog evidence.
3. The correlator confirms or prunes hypotheses. Unknown schema, duplicate-index, ambiguous, and
   destructive-request paths are handled explicitly.
...
OpenAI is switchable with `LLM_PROVIDER`, and deterministic mode powers reproducible offline evals.
...
evals/          deterministic runner, scorer, JSON and Markdown results
